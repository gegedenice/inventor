...
description: "Case study of PRINCE (Bayer + Thoughtworks): a production agentic RAG + Text-to-SQL system over decades of preclinical PDF reports, framed through context engineering and harness engineering."
...
an [O’Reilly course](https://www.oreilly.com/live-events/building-reliable-rag-applications-from-poc-to-production/0642572012347/) on building production-ready RAG applications.
...
combining the generative power of LLMs with the precision of information retrieval systems, [Retrieval-Augmented Generation](https://martinfowler.com/articles/gen-ai-patterns/#rag) (RAG) has emerged as a promising technique.
...
stores, LLM fallbacks, and observability—while [Figure 2](#new-research-workflow.png) zooms into how the system coordinates its specialized agents.
...
PRINCE incorporates an agentic [RAG](https://martinfowler.com/articles/gen-ai-patterns/#rag) system ([Figure 2](#new-research-workflow.png)) to handle complex user requests that require multiple
...
a [hybrid retriever](https://martinfowler.com/articles/gen-ai-patterns/#hybrid-retriever) approach focused on two distinct
patterns:
...
phrasing and terminology, [query expansion](https://martinfowler.com/articles/gen-ai-patterns/#query-rewrite) (multi
...
- Hybrid Retriever: information retrieval from the vector database ([Amazon OpenSearch Service](https://aws.amazon.com/opensearch-service/)) utilizes a Hybrid Search approach that combines metadata filtering,
...
- Reranking: the initial set of retrieved chunks (k=~20) is then refined using a [Rerank](https://martinfowler.com/articles/gen-ai-patterns/#reranker) step. A cross-encoder model (bge-reranker-large)
...
completeness and
presentation*, complementing the Reflection Agent's focus on *data sufficiency* earlier in the workflow. Importantly, all outputs from these regulatory drafting workflows
...
Relevancy* (how well the answer addresses the query), *Context Relevancy* (relevance of retrieved chunks), *Answer Accuracy* (comparison to ground truth),
and *Semantic
